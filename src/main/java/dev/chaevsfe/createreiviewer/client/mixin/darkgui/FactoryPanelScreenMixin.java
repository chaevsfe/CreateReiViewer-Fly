...
public abstract class FactoryPanelScreenMixin {
    @ModifyConstant(method = "renderWindow(Lnet/minecraft/client/gui/GuiGraphicsExtractor;IIF)V", constant = @Constant(intValue = 0xFF3D3C48))
    private int createreiviewer$darkTitle(int original) {
        return DarkGui.text(original, DarkGui.PALE_BLUE);
    }

    @ModifyConstant(method = "init()V", constant = @Constant(intValue = 0xFF555555))
